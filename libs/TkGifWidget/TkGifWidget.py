...
class AnimatedGif(tk.Frame):
    def __init__(
        ...
        # 创建动图的容器 
        self.img_container = tk.Label(self, image=None, bd=0, bg=self.bg)
    ...
    def set_gif(self, file_path):
        ...
        if file_path:
            ...
            with Image.open(file_path) as im:
                ...
                self.duration_lst = []
                for i in sequence:
                    # 确保 image_lst 中存储的是 PIL 图像对象
                    self.image_lst.append(i.copy())
    ...
    def _next_frame(self):
        ...
        if img == -1 or duration == -1:
            ...
            # 将 PIL 图像转换为 Tkinter 可以识别的 PhotoImage
            new_photo_image = ImageTk.PhotoImage(img)
            # 更新 Label 控件上的图像
            self.img_container.configure(image=new_photo_image)
            # 保存对新 PhotoImage 的引用，避免 Tkinter 垃圾回收问题
            self.img_container.image = new_photo_image
        # 设置定时器以继续播放下一帧
        self._cbid = self.after(duration, self._next_frame)
